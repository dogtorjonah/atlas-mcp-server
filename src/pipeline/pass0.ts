...
import { replaceImportEdges, upsertPass0Record } from '../db.js';
...
export async function runPass0(
  ...
  for (const absolutePath of sourceFiles) {
    ...
    // Always resume-safe: only update structural fields, preserve existing AI data.
    // Even with --force, we keep the old data intact until new extractions overwrite it.
    // This means a crash during --force never leaves orphaned empty records.
