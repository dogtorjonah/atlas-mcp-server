/**
 * Legacy: Anthropic LLM provider for extraction pipeline.
 *
 ...
 * (via Voyage AI). In the default heuristic-only pipeline, these capabilities
  ...
  return {
    ...
    async generateBlurb({ filePath, sourceText }): Promise<string> {
      ...
      const payload = await postJson<{
        ...
        max_tokens: 256,
        temperature: 0.1,
        system: 'You are a senior software architect writing concise file summaries.',
        messages: [
          {
            role: 'user',
