...
export function registerCyclesTool(server: McpServer, runtime: AtlasRuntime): void {
  toolWithDescription(server)(
    ...
    'Detect circular dependency cycles using Tarjan\'s strongly connected components algorithm. Finds groups of files that form import loops. Best for: breaking circular dependencies, understanding tangled modules, refactor planning.',
    {
      ...
      min_size: z.number().int().min(2).max(MAX_MIN_SIZE).optional(),
      minSize: z.number().int().min(2).max(MAX_MIN_SIZE).optional(),
      include_test_files: z.boolean().optional(),
      includeTestFiles: z.boolean().optional(),
      max_nodes: z.number().int().min(1).max(MAX_NODES).optional(),
      maxNodes: z.number().int().min(1).max(MAX_NODES).optional(),
      max_edges: z.number().int().min(1).max(MAX_EDGES).optional(),
      maxEdges: z.number().int().min(1).max(MAX_EDGES).optional(),
