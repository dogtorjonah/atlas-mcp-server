...
function symbolUsedAnywhere(symbol: string, files: AtlasFileRecord[]): boolean {
  for (const file of files) {
    ...
    if (collectCrossRefTexts(file).some((text) => includesSymbolByContext(text, symbol))) return true;
  }
  return false;
}
...
function symbolUsedInRow(symbol: string, row: AtlasFileRecord): boolean {
  ...
  if (symbols?.[symbol] && symbols[symbol].total_usages > 0) return true;
  return collectCrossRefTexts(row).some((text) => includesSymbolByContext(text, symbol));
}
...
async function runGapsAction(
  ...
  if (types.includes('imported_not_used')) {
    for (const edge of edges) {
      ...
      const usedAnyIdentifier = [...importedIdentifiers].some((name) => includesSymbolByContext(bodyWithoutImports, name));
      const usedAnyTargetSymbol = [...symbolCandidates].some((symbol) => includesSymbolByContext(bodyWithoutImports, symbol));
...
export function registerAuditTool(server: McpServer, runtime: AtlasRuntime): void {
  toolWithDescription(server)(
    ...
    ].join('\n'),
    {
      ...
      include_test_files: z.boolean().optional(),
      ...
      min_severity: z.number().int().optional(),
      minSeverity: z.number().int().optional(),
